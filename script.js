...
/* Property array will hold the properies, Unit array will hold all units related to that particular property and Factor  */
...
/* Temperature required increment as well as a multiplying factor */
...
temperature = [0, -32, -273.15, -491.67]
...
/* --------- Update all menus ------------- */
const UpdateMenu = (propMenu, unitMenu) => {
    document.form_a.unit_input.value = "";
    document.form_b.unit_input.value = "";
    var index = propMenu.selectedIndex;
    FillUnitMenu(unit[index], unitMenu);
}

/* --------- Fill all arrays values to the options --------*/
const FillUnitMenu = (unitNames, unitMenu) => {
    unitMenu.length = unitNames.length // setting length
    for (let i = 0; i < unitNames.length; i++) {
        unitMenu.options[i].text = unitNames[i]
    }
}

const CalculateUnit = (srcForm, targetForm) => {
    var val = srcForm.unit_input.value;
    if (val == 0 || val != undefined) {
        srcForm.unit_input.value = val;
        CalculateAns(srcForm, targetForm)
    }
}

const CalculateAns = (srcForm, targetForm) => {
    var propIndex;
    var srcIndex, targetIndex;
    var srcFact, targetFact;
    var res;

    propIndex = document.property.menu.selectedIndex;
    srcIndex = srcForm.unit_menu.selectedIndex;
    srcFact = factor[propIndex][srcIndex]; // factor of selected property and selected src

    targetIndex = targetForm.unit_menu.selectedIndex;
    targetFact = factor[propIndex][targetIndex];

    /* Convert to base unit */
    res = srcForm.unit_input.value * srcFact
    if (property[propIndex] == "Temperature")
        res += temperature[srcIndex];

    /* Convert to required form */
    res = res / targetFact
    if (property[propIndex] == "Temperature")
        res -= temperature[targetIndex];

    targetForm.unit_input.value = res

}

window.onload = () => {
    FillUnitMenu(property, document.property.menu);
    UpdateMenu(document.property.menu, document.form_a.unit_menu)
    UpdateMenu(document.property.menu, document.form_b.unit_menu)
}

const openCalc = () => document.getElementById('calc-modal').style.display = 'block'
const closeCalc = () => document.getElementById('calc-modal').style.display = 'none'
...
Array.from(buttons).forEach((btn) => {
    btn.onclick = (e) => {
        let v = document.querySelector('.calc-input-field');
        if (e.target.innerHTML == '=') {
            val = eval(val);
            v.value = val;
        }
        else if (e.target.innerHTML == "USE") {
            if (val == "") alert('No value')
            document.form_a.unit_input.value = val;
            v.value = "";
            val = ""
            closeCalc();
        }
